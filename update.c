...
auto_resolve_entry_point:
	...
	if ( force ) {
		...
		if (last_conn_status == CR_OK_PARENT_DIR_MISSING) {
			int filename_length=strlen(filename);
			char parent_dirname[filename_length];
			struct stat sb;
			struct textlist *tl = 0, *t;

			get_parent_child_from_path(parent_dirname,NULL,filename);
			csync_debug(2,"missing parent dir : %s",parent_dirname);

			SQL_BEGIN("Query File Table for missing files on the peer","SELECT filename from file where filename = '%s' UNION ALL SELECT filename from file where filename > '%s/' and filename < '%s0' ORDER BY filename;",url_encode(parent_dirname),url_encode(parent_dirname), url_encode(parent_dirname))
			{
